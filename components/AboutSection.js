...
import { fadeIn, staggerContainer, slideIn } from './ui/animations';
import { GradientText } from './ui/AnimatedText';
import { resumeData } from '../utils/resumeData';
...
const AboutSection = () => {
  ...
  // Extract data from resumeData
  const { personalInfo, summary, strengths } = resumeData;
  ...
  const personalInfoItems = useMemo(() => [
    ...
    { 
      icon: <Gem size={20} color={colors.primary} />, 
      label: "Location:", 
      value: `${personalInfo.location.city}, ${personalInfo.location.state}` 
    }
  ], [colors.primary, personalInfo]);
      ...
      <Container maxWidth="lg" sx={{ position: 'relative', zIndex: 1 }}>
        ...
        >
          ...
          <Grid item xs={12} md={6}>
            <Box component={motion.div} variants={fadeIn('right', 0.3)}>
              <SectionHeader 
                icon={<Code size={24} />}
                title="About Me"
              ...
              <InterestsCard interests={interests} theme={theme} />
          ...
          {/* Right column - Strengths and info cards */}
          <Grid item xs={12} md={6}>
            <Box component={motion.div} variants={fadeIn('left', 0.3)}>
              <SectionHeader 
                icon={<Shield size={24} />}
                title="My Strengths"
                colors={colors}
              ...
              <StrengthsList strengths={strengths} theme={theme} />
              ...
              <PersonalInfoGrid items={personalInfoItems} theme={theme} />
            </Box>
          </Grid>
        </Grid>
      </Container>
...
const BackgroundElements = React.memo(({ theme }) => {
  const isDark = theme.palette.mode === 'dark';
  const accentColor = isDark ? 'rgba(157, 101, 255, 0.1)' : 'rgba(110, 69, 226, 0.1)';
      ...
      {[1, 2, 3, 4].map((i) => (
        ...
        >
          <Gem
            size={24 + Math.random() * 20}
            color={isDark 
              ? `rgba(157, 101, 255, ${0.2 + Math.random() * 0.3})`
              : `rgba(110, 69, 226, ${0.2 + Math.random() * 0.3})`}
          />
        </Box>
      ))}
    </>
...
const SectionHeader = React.memo(({ icon, title, subtitle, colors, sx }) => (
  <Box sx={{ textAlign: 'center', mb: 4, ...sx }}>
    {subtitle && (
      ...
      >
        ...
        {subtitle}
        {React.cloneElement(icon, { sx: { ml: 1 } })}
      </Typography>
    )}
    <Typography
      variant="h2"
...
const SummaryCard = React.memo(({ summary, theme }) => (
  <Paper
    component={motion.div}
...
const InterestsCard = React.memo(({ interests, theme }) => (
  <Paper
    component={motion.div}
...
const ActionButtons = React.memo(({ theme }) => (
  <Box
    component={motion.div}
...
const StrengthsList = React.memo(({ strengths, theme }) => (
  <Grid container spacing={2} sx={{ mb: 4 }}>
    {strengths.map((strength, index) => (
      <Grid item xs={12} key={index}>
        <Paper
...
const PersonalInfoGrid = React.memo(({ items, theme }) => (
  <Grid container spacing={2}>
